create table ndnp.batch (pkey int8 not null, awardee_key int8, primary key (pkey));
create table ndnp.batch_lccn (package_key int8 not null, lccn_key int8 not null, primary key (package_key, lccn_key));
create table ndnp.batch_reel (package_key int8 not null, reel_key int8 not null, primary key (package_key, reel_key));
create table ndnp.lccn (pkey int8 not null, lccn varchar(255) not null, primary key (pkey), unique (lccn));
create table ndnp.reel (pkey int8 not null, reelnumber varchar(255) not null, primary key (pkey), unique (reelnumber));
