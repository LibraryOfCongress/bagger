...
config = {
    ...
    'HOST': 'einsteinium', #The hostname, e.g., localhost or ac, that the service container is to be exposed under.  Also used to identify the responder.  Default is localhost.    
    ...
    'GROUP':'qatransfer', #Set the group that the service container should run as (default = 'transfer')
    'RUN_NUMBER':'', #Set the run number for the service container (default = 85)
    'COMPONENTS_PROPS': {
#        'components.zfsfilesystemcreator.pool':'zpool',
        'components.archivalremotedirectorycopier.keyfile':'/home/qatransfer/.ssh/id_rsa',
        'components.archivalremotedirectorycopier.staging.basepath':'/vol/ndnp/transfer_qa/archive_tmp',
        'components.archivalremotedirectorycopier.remote.user':'qatransfer'
    }                
}
